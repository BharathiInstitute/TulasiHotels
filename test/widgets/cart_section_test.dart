...
import 'package:tulasihotels/features/billing/widgets/cart_section.dart';
...
void main() {
  ...
  Widget buildCart({
    ...
    return ProviderScope(
      overrides: [cartProvider.overrideWith((_) => notifier)],
      child: MaterialApp(
        home: Scaffold(
          body: SizedBox(
            height: 600,
            child: CartSection(onPay: onPay ?? () {}, showHeader: showHeader),
          ),
        ),
      ),
    );
  ...
  group('CartSection', () {
    ...
    testWidgets('shows empty state when cart is empty and showHeader is true', (
      ...
      expect(find.text('Order is empty'), findsOneWidget);
      expect(find.text('Tap menu items to add'), findsOneWidget);
      expect(find.byIcon(Icons.receipt_long_outlined), findsOneWidget);
    });

    testWidgets('shows Order header with showHeader', (tester) async {
      await tester.pumpWidget(buildCart(showHeader: true));
    ...
    testWidgets('renders cart items', (tester) async {
      await tester.pumpWidget(
        buildCart(products: sampleProducts, showHeader: true),
      );
    ...
    testWidgets('shows item count in collapsed header', (tester) async {
      ...
      // collapsed header shows "ORDER (N items)"
