/// Tests for WindowsUpdateService â€” version comparison and model logic
///
...
void main() {
  ...
  group('Silent attempt escalation logic', () {
    ...
    test('0 attempts â€” still in silent mode', () {
      const attempts = 0;
      expect(attempts >= maxSilentAttempts, false);
    });

    test('1 attempt â€” still in silent mode', () {
      const attempts = 1;
      expect(attempts >= maxSilentAttempts, false);
    });

    test('2 attempts â€” still in silent mode', () {
      const attempts = 2;
      expect(attempts >= maxSilentAttempts, false);
    });

    test('3 attempts â€” escalate to dialog', () {
      const attempts = 3;
      expect(attempts >= maxSilentAttempts, true);
    });

    test('5 attempts â€” stays at dialog', () {
      const attempts = 5;
      expect(attempts >= maxSilentAttempts, true);
    });
  });
  ...
  group('MSIX detection logic', () {
    ...
    test('WindowsApps path should be detected as MSIX', () {
      const exePath =
          r'C:\Program Files\WindowsApps\39648TulasiERP.TulasiStores_1.0.4.0\tulasihotels.exe';
      expect(exePath.contains('WindowsApps'), true);
    });

    test('Program Files path should NOT be detected as MSIX', () {
      const exePath = r'C:\Program Files\TulasiStores\tulasihotels.exe';
      expect(exePath.contains('WindowsApps'), false);
    });

    test('AppData install path should NOT be detected as MSIX', () {
      const exePath =
          r'C:\Users\user\AppData\Local\Programs\TulasiStores\tulasihotels.exe';
      expect(exePath.contains('WindowsApps'), false);
    });

    test('case sensitivity â€” WindowsApps is exact match', () {
      const exePath = r'C:\Program Files\windowsapps\tulasihotels.exe';
      ...
      expect(exePath.contains('WindowsApps'), false);
      // Lowercase variant won't match â€” this is expected and safe because
  ...
  group('Dual-distribution behavior', () {
    ...
    test('Inno exe install should run custom update check', () {
      ...
      const shouldSkip = !isWindows || isMsix;
      expect(shouldSkip, false); // Should NOT skip â€” run update check
    });
