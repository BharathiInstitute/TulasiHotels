        ...
        try {
            ...
            switch (event.event) {
                ...
                case "subscription.charged": {
                    ...
                    const chargedSubId = event.payload.subscription?.entity?.id;
                    if (!chargedSubId) break;
                ...
                case "subscription.halted": {
                    ...
                    const haltedSubId = event.payload.subscription?.entity?.id;
                    if (!haltedSubId) break;
...
export const activateSubscription = functions
    ...
    .https.onCall(async (data: {
        ...
        try {
            ...
            const payment = await verifyRes.json() as { status: string; amount: number };
...
export const processReferralReward = functions
    ...
    .onCreate(async (snapshot) => {
        ...
        const userId: string | undefined = subscriptionData?.userId;
        if (!userId) return;
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            await db.runTransaction(async (txn) => {
                ...
                const billsLimit = (limits.billsLimit as number) || 300;
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            await db.runTransaction(async (txn) => {
                ...
                const productsLimit = (limits.productsLimit as number) || 50;
 ...
 * Validates limit and deletes if over (safety net).
 */
export const onTableCreated = functions
    .region("asia-south1")
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            await db.runTransaction(async (txn) => {
                ...
                const tablesLimit = (limits.tablesLimit as number) || 5;
 ...
 */
export const onTableDeleted = functions
    .region("asia-south1")
    .firestore.document("users/{userId}/tables/{tableId}")
    .onDelete(async (_snap, context) => {
        const db = admin.firestore();
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            await db.runTransaction(async (txn) => {
                ...
                const data = userDoc.data()!;
                const limits = data.limits || {};
                const staffCount = (limits.staffCount as number) || 0;
                const staffLimit = (limits.staffLimit as number) || 0;
