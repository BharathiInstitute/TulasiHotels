...
class _SettingsWebScreenState extends ConsumerState<SettingsWebScreen> {
  ...
  void _subscribeToLimits() {
    try {
      ...
      // Ignore Firebase access in test/offline contexts where no app is initialized.
    }
  }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: (success || isOffline)
                ? AppColors.primary
                : AppColors.error,
          ),
        );
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_showsGlobalSaveButton(_selectedTab))
            TextButton.icon(
              onPressed: _isSaving || !selectedPermission.canUpdate
                  ? null
                  : _saveSettings,
              icon: _isSaving
                  ? const SizedBox(
        ...
        _SectionCard(
          ...
          child: Column(
            ...
            children: [
              ...
              // Connection section for Web Serial
              if (printerState.printerType == PrinterTypeOption.webSerial) ...[
                _buildFieldLabel('USB Serial Printer'),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(
                    ...
                    ElevatedButton.icon(
                      onPressed: () async {
                        final success = await WebSerialPrinterService.connect();
                        if (mounted) {
                          setState(() {});
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Uses the browser print dialog. Click Print on a receipt to open it.',
              ...
              Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<double>(
                    initialValue:
                        [0.0, 5.0, 12.0, 18.0].contains(_selectedGstRate)
                        ? _selectedGstRate
                        : 5.0,
            ...
            if (isWindowsDesktop) {
              ...
              Future<void> refreshVerificationStatus() async {
                ...
                final doc = await FirebaseFirestore.instance
                    ...
                    .get();
                final verified =
                    (doc.data()?['phoneVerified'] as bool?) ?? false;
