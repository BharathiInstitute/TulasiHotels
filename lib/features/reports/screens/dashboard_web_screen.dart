...
import 'package:tulasihotels/models/sales_summary_model.dart';
...
_Generated by ${ref.read(currentUserProvider)?.shopName ?? AppConstants.appName}_
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = context.l10n;
    final dashboardPermission = ref.watch(
      routePermissionProvider(AppRoutes.dashboard),
    );

    if (!dashboardPermission.isResolved) {
    ...
    if (!dashboardPermission.canView) {
      return Scaffold(
        body: PermissionDeniedView(
          message: PermissionCenter.deniedViewMessage(AppRoutes.dashboard),
        ),
      );
    }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Quick Access — linked features
            _QuickAccessSection(isMobile: isMobile),
...
class _QuickAccessSection extends ConsumerWidget {
  final bool isMobile;
  const _QuickAccessSection({required this.isMobile});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final theme = Theme.of(context);
    final groups = <_QuickGroup>[
      const _QuickGroup('Reports', Icons.bar_chart, [
        _QuickItem(
          'Menu Performance',
          Icons.restaurant_menu,
          AppRoutes.menuPerformance,
        ),
        _QuickItem('Weekly Report', Icons.date_range, AppRoutes.weeklyReport),
        _QuickItem('P&L Report', Icons.account_balance, AppRoutes.pnlReport),
        _QuickItem('Peak Hours', Icons.access_time, AppRoutes.peakHours),
        _QuickItem('Item Sales', Icons.receipt_long, AppRoutes.itemSales),
        _QuickItem('Comparative', Icons.compare_arrows, AppRoutes.comparative),
        _QuickItem(
          'Feedback Report',
          Icons.star_rate,
          AppRoutes.feedbackReport,
        ),
        _QuickItem('GST Export', Icons.description, AppRoutes.gstExport),
      ]),
      const _QuickGroup('Inventory', Icons.inventory_2, [
        _QuickItem('Ingredients', Icons.egg, AppRoutes.ingredients),
          ...
          Wrap(
            ...
            children: groups.map((group) {
              final visibleItems = group.items.where((item) {
                final permission = ref.watch(routePermissionProvider(item.route));
...
class _QuickGroup {
  ...
  const _QuickGroup(this.label, this.icon, this.items);
}
